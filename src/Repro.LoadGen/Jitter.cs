...
/// belt-and-braces rather than the thing standing between you and a spin loop. Two config
/// blocks are validated by that rule now (<c>simple.jitter</c> and
/// <c>simpleActivity.jitter</c>), so two copies of the formula plus two copies of the
/// prose explaining which rule makes it safe is how the next change to the jitter
/// contract lands in one loop and not the other. The symptom is a busy spin against the
/// frontend in exactly one driver.
/// <para>
