...
filescan = grid([
    ...
    dict(title="Memory: managed heap, LOH, working set", unit=BYTES, minval=0,
         desc="Three PROCESS-WIDE gauges, and every reading a newcomer gets wrong about "
              ...
              "MiB for the 500 MB corpus, in ONE sample, and the LOH is not compacted, so it "
    ...
    dict(title="GC collections /s by generation", unit=CPS, minval=0,
         desc="A watermark-backed counter: the activity samples GC.CollectionCount(g) once "
              ...
              "`gen` EARNS ITS LABEL, and it is the counter-example to the memory panel "
              "above: gen values PARTITION the quantity -- a gen0 collection is not a gen2 "
              "collection -- while those byte gauges NEST. That is the whole rule on this "
              "board: a label when the values partition, separate names when they nest.\n\n"
